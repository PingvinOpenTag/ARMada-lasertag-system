...
void show_menu(tmenu* menu){
	...
	for(uint8_t i=0; i < menu->item_list->count; i++){
		...
		if(xSemaphoreTake(xColorLCDSemaphore, (portTickType)(TIC_FQR*2)/*600*/ )== pdTRUE)//если LCD занят, ждем 2 с
		{
			PStr(menu->item_list->list[i]->text, &TS);
			xSemaphoreGive(xColorLCDSemaphore);
		}
#endif
	}
...
#ifndef COLOR_LCD
	lcd8544_putstr(0, 0, settings->header_text , 0); // вывод заголовка
#else
	volatile TextParamStruct TS;
	TS.Size = 0;
	TS.Font = StdFont;
...
#ifndef COLOR_LCD
		lcd8544_putstr(settings->item_list->list[i]->position.X, settings->item_list->list[i]->position.Y, settings->item_list->list[i]->text , 0);
#else
		TS.XPos = settings->item_list->list[i]->position.X;
		TS.YPos = settings->item_list->list[i]->position.Y;
		if(xSemaphoreTake(xColorLCDSemaphore, (portTickType)(TIC_FQR*2)/*600*/ )== pdTRUE)//если LCD занят, ждем 2 с
		{
			PStr(settings->item_list->list[i]->text, &TS);
			xSemaphoreGive(xColorLCDSemaphore);
		}
...
#ifndef COLOR_LCD
					lcd8544_putstr(settings->item_list->list[i]->position.X+7*symbol_width, settings->item_list->list[i]->position.Y, int_to_str(*((uint8_t*)settings->item_list->list[i]->new_value_address),settings->item_list->list[i]->digits) , 0);
#else
		TS.XPos = settings->item_list->list[i]->position.X+7*symbol_width;
		TS.YPos = settings->item_list->list[i]->position.Y;
		if(xSemaphoreTake(xColorLCDSemaphore, (portTickType)(TIC_FQR*2)/*600*/ )== pdTRUE)//если LCD занят, ждем 2 с
		{
			PStr(int_to_str(*((uint8_t*)settings->item_list->list[i]->new_value_address),settings->item_list->list[i]->digits), &TS);
			xSemaphoreGive(xColorLCDSemaphore);
		}
#endif
				}
...
#ifndef COLOR_LCD
					lcd8544_putstr(settings->item_list->list[i]->position.X+7*symbol_width, settings->item_list->list[i]->position.Y, long_int_to_str(*((uint16_t*)settings->item_list->list[i]->new_value_address),settings->item_list->list[i]->digits) , 0);
#else
					TS.XPos = settings->item_list->list[i]->position.X+7*symbol_width;
					TS.YPos = settings->item_list->list[i]->position.Y;
					if(xSemaphoreTake(xColorLCDSemaphore, (portTickType)(TIC_FQR*2)/*600*/ )== pdTRUE)//если LCD занят, ждем 2 с
					{
						PStr(long_int_to_str(*((uint16_t*)settings->item_list->list[i]->new_value_address),settings->item_list->list[i]->digits), &TS);
						xSemaphoreGive(xColorLCDSemaphore);
					}
#endif
				}
...
#ifndef COLOR_LCD
	lcd8544_putstr(settings->item_list->list[settings->curr_item]->position.X - symbol_width, settings->item_list->list[settings->curr_item]->position.Y, ">" , 0);
	if (!screen_auto_refresh) lcd8544_dma_refresh(); // вывод буфера на экран ! без этого ничего видно не будет !
#else
			...
			switch(keyboard_event){
		  			...
		  			case key_pressing:
		  			{
		  				if (settings->curr_item == settings->apply_index)
		  				{
		  					...
		  					RTC_SetCounter(FtimeToCounter(&date));
		  					page_event = _APPLY;
		  					break;
		  				}
		  					...
		  					switch(settings->item_list->list[settings->curr_item]->var_type){
		  									case _BYTE:
		  									{
		  										...
		  										lcd8544_putstr(settings->item_list->list[settings->curr_item]->position.X+7*symbol_width, settings->item_list->list[settings->curr_item]->position.Y, int_to_str(*((uint8_t*)settings->item_list->list[settings->curr_item]->new_value_address),settings->item_list->list[settings->curr_item]->digits) , 0);
		  						  				...
		  						  				if(xSemaphoreTake(xColorLCDSemaphore, (portTickType)(TIC_FQR*2)/*600*/ )== pdTRUE)//если LCD занят, ждем 2 с
		  						  				{
		  						  					PStr(int_to_str(*((uint8_t*)settings->item_list->list[settings->curr_item]->new_value_address),settings->item_list->list[settings->curr_item]->digits), &TS);
		  						  					xSemaphoreGive(xColorLCDSemaphore);
		  						  				}
#endif
		  									...
		  									case _YEAR:
		  									{
		  										...
		  										lcd8544_putstr(settings->item_list->list[settings->curr_item]->position.X+7*symbol_width, settings->item_list->list[settings->curr_item]->position.Y, long_int_to_str(*((uint16_t*)settings->item_list->list[settings->curr_item]->new_value_address),settings->item_list->list[settings->curr_item]->digits) , 0);
		  						  				...
		  						  				if(xSemaphoreTake(xColorLCDSemaphore, (portTickType)(TIC_FQR*2)/*600*/ )== pdTRUE)//если LCD занят, ждем 2 с
		  						  				{
		  						  					PStr( long_int_to_str(*((uint16_t*)settings->item_list->list[settings->curr_item]->new_value_address),settings->item_list->list[settings->curr_item]->digits), &TS);
		  						  					xSemaphoreGive(xColorLCDSemaphore);
		  						  				}
#endif
...
void init_item_list(void){
	...
	main_menu.header_text=main_menu_header_text;
...
void init_sett_list(void){
	...
	sett_page.header_text=sett_page_header_text;
...
void init_clock_sett_list(void){
	...
	sett_page.header_text=clock_page_header_text;
	sett_page.height = header_height;
	sett_page.item_list = (tsetting_item_list*)& sett_item_list;
	sett_page.curr_item = 0;
...
void add_menu_item(const tmenu_item_list* itm_list, const tmenu_item* menu_itm){
	...
	plist = itm_list;
	plist->list[plist->count]= menu_itm;
	plist->count +=1;
...
void add_setting_item(const tsetting_item_list* itm_list, const tsetting_item* setting_itm){
	...
	plist = itm_list;
	plist->list[plist->count]= setting_itm;
	plist->count +=1;
}
