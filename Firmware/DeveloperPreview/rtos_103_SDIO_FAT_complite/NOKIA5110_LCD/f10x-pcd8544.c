...
void drawDigit(int16_t x, int16_t y, char digit, unsigned char mode){
	...
	if (mode)
	{
		 ...
		 drawBitmap(x,y,(char *)&alternative_digits[digit][0],12,16,1);
...
void drawSmallDigit(int16_t x, int16_t y, char digit, unsigned char mode){
	...
	{
		 fillRect(x,y,4,8,0);
		 drawBitmap(x,y,(char *)&small_digits[digit][0],4,8,1);
...
void lcd_rounds_update(void){
	...
	if(virtual_rounds>0)
	{
		...
		{
			drawBitmap(0,16,(char*)&clip_img[virtual_rounds][0],16,24,1);
		}
		else{
			//fillRect(0,16,16,24,0);
			drawBitmap(0,16,(char*)&clip_img[0][0],16,24,1);
		}
	}
	else{

		if (armadaSystem.gun.rounds>0)drawBitmap(0,16,(char*)&clip_img[1][0],16,24,1);
		else drawBitmap(0,16,(char*)&clip_img[0][0],16,24,1);
...
void lcd_fire_mode_update(tfire_mode fire_mode)
{
	fillRect(84-6*4-2,48-8,6*4,8,0);//удаляем изображение прогресс-бара в батарейке
			switch(fire_mode)
			{
				case SINGLE:
				{
					lcd8544_putstr(84-6*4-2,48-8 ,"SING", 1); // вывод первой строки
				}
				break;
				case SEMI_AUTO:
				{
					lcd8544_putstr(84-6*4-2,48-8 ,"SEMI", 1); // вывод первой строки
				}
				break;
				case AUTO:
				{
					lcd8544_putstr(84-6*4-2,48-8 ,"AUTO", 1); // вывод первой строки
				}
				break;
				default: break;
