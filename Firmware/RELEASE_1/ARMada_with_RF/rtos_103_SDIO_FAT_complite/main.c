...
//	  initGUI();
	...
	if(voltage > 3400) //проверка состояния аккумулятора
	{
		...
		memcpy((void*)armadaSystem.wav_player.message_sound_file_name,"/syssnd/battok.wav",strlen("/syssnd/battok.wav"));
		armadaSystem.wav_player.message_sound_file_name[strlen("/syssnd/battok.wav")] = 0;
	...
	else
	{
		...
		/*
		armadaSystem.wav_player.type_of_sound_to_play = USER_SOUND;
		memcpy((void*)armadaSystem.wav_player.message_sound_file_name,"/syssnd/batlow.wav",strlen("/syssnd/batlow.wav"));
		armadaSystem.wav_player.message_sound_file_name[strlen("/syssnd/batlow.wav")] = 0;
		xSemaphoreGive(xWavPlayerManagerSemaphore);
		*/
	}
	...
	if((armadaSystem.bluetooth.cmode[0]=='1')&&(armadaSystem.bluetooth.role[0]=='1')){
		bluetooth_clear_bind_list();
	}
	else bt_reset();
...
if(game_status==OUT_OF_GAME){
	taskENTER_CRITICAL();
	sensors[zone_1]=index_to_color[armadaSystem.player.team_color];
	sensors[zone_2]=index_to_color[armadaSystem.player.team_color];
	sensors[zone_3]=index_to_color[armadaSystem.player.team_color];
	sensors[zone_4]=index_to_color[armadaSystem.player.team_color];
	taskEXIT_CRITICAL();
}
...
void bluetooth_clear_bind_list(void){
	bool at_res;
	USART_DeInit(USART1);
	InitCOM1(38400);
...
/*
		...
		if(armadaSystem.bluetooth.cmode[0]=='1')
		{
			at_res = send_at_command((char*)at_rmaad);
#ifdef DISPLAY_ENABLE
		if(!((line_counter+1)<line_on_screen)){
		...
		display_put_string(0,display_standard_symbol_height()*(line_counter++),at_rmaad);
#endif
		if (at_res)
					{
#ifdef DISPLAY_ENABLE
...
*/
	...
	}//[if(strlen((const char*)armadaSystem.bluetooth.cmode))]
