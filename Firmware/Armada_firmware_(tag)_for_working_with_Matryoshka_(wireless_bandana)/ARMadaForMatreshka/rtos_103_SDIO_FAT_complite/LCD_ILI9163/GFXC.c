...
//		    offset = (((uint16_t)data[11])<<8)|(uint16_t)data[10];
    ...
    SB((0x08|(1<<7)), Dat);
   	...
   	SetAddrForBMP(0, 0, size_x-1, size_y-1);
			 while(n!=0){
				 if (n <= pix_buffer){
					 f_read(&fbmp, &pix_buff[0], n*3, (UINT *)&len);
					 n=0;
				 }

				 else{

					 f_read(&fbmp, &pix_buff[0], pix_buffer*3, (UINT *)&len);
...
//	taskENTER_CRITICAL();
	...
	PNum((int32_t)integer,(TextParamStruct *) &TS);
	PStr(".",  (TextParamStruct *)&TS);
	PNum((int32_t)tenths,(TextParamStruct *) &TS);
	PNum((int32_t)hundredths, (TextParamStruct *)&TS);
	PStr("V", (TextParamStruct *)&TS);
	drawBMPfromFlashUseDMA((uint16_t *)ppic,65,0,20,64,Rotation_90);
//	taskEXIT_CRITICAL();
	xSemaphoreGive(xColorLCDSemaphore);
	 }
}
...
//	taskENTER_CRITICAL();
	...
	TS.Precision = 2;
	TS.Padding = 2;
	PNum((int32_t)armadaSystem.player.health, (TextParamStruct *)&TS);
...
//		taskENTER_CRITICAL();
		...
		PNum((int32_t)armadaSystem.gun.rounds, (TextParamStruct *)&TS);
...
//			taskENTER_CRITICAL();
			...
			PNum((int32_t)armadaSystem.gun.clips,(TextParamStruct *) &TS);
...
//	taskENTER_CRITICAL();
				...
				TS.Precision = 1;
				TS.Padding = 1;
				if((date.month>0)&&(date.month <13))PStr((const char*)monthsNamesPointers[date.month-1], (TextParamStruct *)&TS);
				if ((date.month>4)&&(date.month<8))	PStr(" ", (TextParamStruct *)&TS);
				PNum((int32_t)date.day,(TextParamStruct *) &TS);
				TS.XPos = time_text_x_pos+4;
				TS.YPos = time_text_y_pos+9;

				PNum((int32_t)date.hour, (TextParamStruct *)&TS);
				PStr(":",(TextParamStruct *) &TS);
				PNum((int32_t)date.minute, (TextParamStruct *)&TS);
	...
	switch(fire_mode)
				{
					case SINGLE:
					{
						PStr("SING", (TextParamStruct *)&TS);
					}
					break;
					case SEMI_AUTO:
					{
						PStr("SEMI", (TextParamStruct *)&TS);
					}
					break;
					case AUTO:
					{
						PStr("AUTO",(TextParamStruct *)&TS);
					}
					break;
					default: break;
