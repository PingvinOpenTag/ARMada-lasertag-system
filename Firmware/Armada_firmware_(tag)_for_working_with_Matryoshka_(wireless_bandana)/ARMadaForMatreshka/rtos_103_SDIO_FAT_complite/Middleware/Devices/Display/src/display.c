...
void display_show_chiter_picter(void){
	...
	clear_screen();
	  ...
	  lcd8544_rect(1,1,83,40,1); // прямоугольник
	  if (!screen_auto_refresh) lcd8544_dma_refresh(); // вывод буфера на экран ! без этого ничего видно не будет !
	  lcd8544_putstr(24, 8, (const unsigned char*)" STOP ", 0); // вывод первой строки
      lcd8544_putstr( 18, 18, (const unsigned char*)"CHEATING", 0); // вывод второй строки
      lcd8544_putstr( 24, 28,(const unsigned char*) "STUPID", 0); // вывод второй строки
	  if (!screen_auto_refresh)lcd8544_dma_refresh();
	  picture_on_screen=CHITER_PIC;

#else
	  if(xSemaphoreTake(xColorLCDSemaphore, (portTickType)(TIC_FQR*2)/*600*/ )== pdTRUE)//если LCD занят, ждем 2 с
	  					 {

	  						drawBMPfromFlashUseDMA((uint16_t*) chiter,0,0,128,128,Rotation_0);
	  						picture_on_screen=CHITER_PIC;
	  						xSemaphoreGive(xColorLCDSemaphore);
	  					 }
...
void display_show_game_over_picture(void){
	picture_on_screen=GAME_OVER_PIC;
#ifndef 	COLOR_LCD
														clear_screen();
