...
//						display_backlight_off();
		...
		FLASH_LED_OFF;
*/
		xQueueSend( xRfRxEventQueue, ( void * ) &rx_package_tmp, TIC_FQR/* portMAX_DELAY*/ );
...
void RF_modul_read_packages(void){
	...
	{//если прочитали
		...
		//	received_unique_packages_array[received_unique_packages++]=package_tmp.details.packageId;
			...
			waiting_interval=0;
			xQueueSend( xRfRxUniquePackagesQueue, ( void * ) &package_tmp, TIC_FQR);
/*
...
void RF_send_status_package(){
tstatus_payload status_payload_tmp;
	status_payload_tmp.type = STATUS_DATA;
	status_payload_tmp.team_color = armadaSystem.player.team_color;
	status_payload_tmp.gun_damage = armadaSystem.gun.damage;
	status_payload_tmp.in_game = game_status;
	status_payload_tmp.bluetooth_is_connected = GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_7);
	status_payload_tmp.player_id = armadaSystem.player.id;
	status_payload_tmp.health = armadaSystem.player.health;
	int ref_adc_val;
	uint32_t voltage;
