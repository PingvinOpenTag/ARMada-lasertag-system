	...
	if (game_status == OUT_OF_GAME)
		{
			if (GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_7) == 1)//если соединение установлено
			{
				...
				memcpy((void*)armadaSystem.wav_player.message_sound_file_name,"/syssnd/nostart.wav",strlen("/syssnd/nostart.wav"));
				armadaSystem.wav_player.message_sound_file_name[strlen("/syssnd/nostart.wav")] = 0;
				xSemaphoreGive(xWavPlayerManagerSemaphore);
			}
		}


		if((countdown_shock_time)||(game_status==OUT_OF_GAME)||(GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_7) == 0))//если врем€ шока не прошло
																				// или внеигры
																				// или нет св€зи
																				//игнорируем нажатие
		return;
