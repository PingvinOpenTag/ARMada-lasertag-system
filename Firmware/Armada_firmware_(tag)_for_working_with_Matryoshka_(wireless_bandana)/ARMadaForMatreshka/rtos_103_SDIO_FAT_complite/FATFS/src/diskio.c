...
)
{
  ...
  switch (drv)
  {
    ...
    {   
       Status = SD_WriteBlock( (uint8_t *)(&buff[0]) ,sector << 9 , 512/*SDCardInfo.CardBlockSize*/);
    }                                                
    else                    /* 多个sector的写操作 */    
    {   
       Status = SD_WriteMultiBlocks( (uint8_t *)(&buff[0]) ,sector << 9 ,512/*SDCardInfo.CardBlockSize*/,count);
    }                                                
    if(Status == SD_OK)
    {
       return RES_OK;
